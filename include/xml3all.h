...
#define XML3_VERSION_REVISION_DATE "01-06-2015"
    ...
    public:
        string tu;
        wstring wu;
        explicit XMLU(const char* x);
        explicit XMLU(const wchar_t* x);
...
// Extra  stuff now
        ...
        explicit xstring(const wchar_t* f, ...)
        {
            va_list args;
            va_start(args, f);
    ...
    {
    public:
        explicit XMLCData(size_t ElementPosition = 0, const char* ht = "");
    ...
    {
    public:
        explicit XMLDocType(const char* ht = "");

        explicit XMLDocType(const wchar_t* ht);
    ...
    public:
        ...
        explicit XMLComment(size_t ElementPosition = 0, const char* ht = "");
    ...
    public:
        ...
        void Clear();
        void Version(XML_VERSION_INFO* x) const;
...
#define _strnicmp strncasecmp
    inline int _vscprintf(const char* format, va_list argptr) { return (vsnprintf(0, 0, format, argptr)); }
    ...
    inline void BXML::ToB(string& s) { s = Char2Base64(p(), d.size()); }

    inline void BXML::FromB(const char* ba) { Base64ToChar(ba, strlen(ba), *this); }

    inline BXML::operator const char*() const { return p(); }

    inline BXML::operator char*() { return p(); }

    inline const char* BXML::p() const { return &d[0]; }

    inline char* BXML::p() { return &d[0]; }

    inline size_t BXML::size() const { return d.size(); }
    ...
    inline void BXML::reset() { d.clear(); }
    ...
    inline void BXML::AddResize(size_t More) { d.resize(d.size() + More); }
    ...
    inline const wchar_t* XMLU::wc() const { return wu.c_str(); }
    inline const char* XMLU::bc() const { return tu.c_str(); }
    inline XMLU::operator const wchar_t*() const { return wu.c_str(); }
    ...
    inline void XMLContent::SetValue(const string& VV) { v = VV; }
    inline void XMLContent::Clear() { v.clear(); }

    inline void XMLContent::SetEP(size_t epp) { ep = epp; }
    ...
    inline const string& XMLContent::GetValue() const { return v; }
    ...
    inline size_t XMLContent::GetEP() const { return ep; }
    ...
    /*
       ...
       XMLContent((size_t)-1,(const char*)0)
                    {
                    operator =(h);
                    }
    ...
    */
    /*
            XMLVariable& XMLVariable :: operator =(const
       std::initializer_list<string>& s)
                    {
                    if (s.size() == 1)
                            v = s.begin()->c_str();
                    if (s.size() == 2)
    ...
    */
    ...
    inline XMLHeader::XMLHeader() { Default(); }
    ...
    inline XMLElement::XMLElement(const char* t) { operator=(t); }
    ...
    inline XMLElement::XMLElement(XMLElement&& from) noexcept
    {
        el = from.el;
        param = from.param;
        cdatas = from.cdatas;
...
// string adsb = Serialize();
    ...
    // Gets
    inline XMLElement& XML::GetRootElement() { return root; }

    inline XMLHeader& XML::GetHeader() { return hdr; }
    ...
    // Sets
    inline void XML::SetRootElement(XMLElement& newroot) { root = newroot; }
    inline void XML::SetHeader(const XMLHeader& h) { hdr = h; }
    ...
    inline void XML::Version(XML_VERSION_INFO* x) const
    {
